...
def receipt(request):
    ...
    # Prepare detailed sales information for the QR Code
    sales_info = {
        'transaction_code': transaction['code'],
        'date': transaction['date_added'].strftime("%Y-%m-%d"),  # Format date to string
    ...
    for item in ItemList:
        sales_info['items'].append({
            'product_id': item.product_id.pk,  # Assuming `product_id` is a ForeignKey field
            'product_name': item.product_id.name,  # Adjust to your model's attribute
            'qty': item.qty,
            'price': item.price,
            'total': item.total,
        })

    # Convert the sales_info dictionary to a JSON string
    sales_info_json = json.dumps(sales_info)
